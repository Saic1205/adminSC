...
import { setsEqual } from "../../cust-utils/equals-set";
import { relativeDateFormatToTimestamp } from "../../cust-utils/time";
...
type VendorListFilterAction =
    | { type: "setQuery"; payload: string | null }
    | { type: "setFilters"; payload: VendorListFilterState }
    | { type: "reset"; payload: VendorListFilterState }
    | { type: "setOffset"; payload: number }
    | { type: "setDefaults"; payload: VendorListDefaultFilters | null };

interface VendorListFilterState {
    query?: string | null;
    status: {
        open: boolean;
    ...
    limit: number;
    offset: number;
    additionalFilters: VendorListDefaultFilters | null;
}
...
const reducer = (
    state: VendorListFilterState,
    action: VendorListFilterAction
): VendorListFilterState => {
    switch (action.type) {
        case "setFilters": {
            return {
                ...state,
...
type VendorListDefaultFilters = {
    expand?: string;
    fields?: string;
};

export const useVendorListFilters = (
    existing?: string,
    defaultFilters: VendorListDefaultFilters | null = null
) => {
    ...
    const initialTabs = useMemo(() => {
        const storageString = localStorage.getItem("vendorLists::filters");
        if (storageString) {
    ...
    const setDefaultFilters = (filters: VendorListDefaultFilters | null) => {
    ...
    const setFilters = (filters: VendorListFilterState) => {
    ...
    const getRepresentationObject = (fromObject?: VendorListFilterState) => {
    ...
    const saveTab = (tabName: string, filters: VendorListFilterState) => {
        ...
        const storedString = localStorage.getItem("vendorLists::filters");
        ...
        if (existing) {
            ...
            localStorage.setItem("vendorLists::filters", JSON.stringify(existing));
        } else {
            const newFilters = {};
            newFilters[tabName] = repString;
            localStorage.setItem("vendorLists::filters", JSON.stringify(newFilters));
        }
...
const parseQueryString = (
    queryString?: string,
    additionals: VendorListDefaultFilters | null = null
): VendorListFilterState => {
    const defaultVal: VendorListFilterState = {
        type: {
            open: false,
            filter: null,
        },
